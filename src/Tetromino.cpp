...
void Tetromino::get_piece(int screen_width) {
    ...
    // 2) in base al tetromino, salvo rotazioni come posizioni in una griglia
    // 4x4 (salvate in binario dentro un numero esadecimale) -> 1 esadecimale
    // per ogni rotazione
    ...
    int        next_piece_random_index =
        random() % piece_data_len; // scelgo pezzo random
    ...
    data = next_piece_data + 1; // salvo INDIRIZZO dell'esadecimale che
                                // rappresenta tetromino(e sua rotazione)
...
int* Tetromino::get_cells() {
    // 8 celle = 8 num binari (di 2 cifre) = 4 coppie = le 4 caselle che il
    // tetr. occupa nella griglia 4x4
    static int cells[8] = {};
...
void Tetromino::move(Moves move) { // cambia le coordinate in base all'input
                                   // (nota:possono essere illegali)
    switch (move) {
    case ROTATE:
...
bool Tetromino::belongs(int check_y,
                        int check_x) { // se coordinata appartiene al tetromino
    int* cells = get_cells();
    int  x_i = 0;
    int  y_i = 0;
    bool flag = false;
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (x_i + this->z == check_x &&
            y_i == check_y) // controlla se appartiene -> nel caso ho risolto
            flag = true;
    }
    // return false;
    return flag;
